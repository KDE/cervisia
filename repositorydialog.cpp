...
public:
    ...
    void setRetrieveCvsignore(bool retrieve)
    {
        m_retrieveCvsignore = retrieve;
    }
    ...
    {
        QString str = text(1);
        return (str.startsWith(QLatin1String("ext (")) ? str.mid(5, str.length() - 6) : QString());
    }
    ...
    bool retrieveCvsignore() const
    {
        return m_retrieveCvsignore;
    }
...
{
    return repository.startsWith(QLatin1String(":pserver:")) || repository.startsWith(QLatin1String(":sspi:"));
}

RepositoryListItem::RepositoryListItem(QTreeWidget *parent, const QString &repo, bool loggedin)
...
{
    QString compressionStr = (compression >= 0) ? QString::number(compression) : i18n("Default");
...
RepositoryDialog::RepositoryDialog(KConfig &cfg,
                                   OrgKdeCervisia5CvsserviceCvsserviceInterface *cvsService,
                                   const QString &cvsServiceInterfaceName,
                                   QWidget *parent)
    : QDialog(parent)
    , m_partConfig(cfg)
    ...
    , m_cvsServiceInterfaceName(cvsServiceInterfaceName)
{
    ...
    m_repoList->setHeaderLabels(QStringList() << i18n("Repository") << i18n("Method") << i18n("Compression") << i18n("Status"));
    m_repoList->setFocus();
    ...
    connect(m_modifyButton, SIGNAL(clicked()), this, SLOT(slotModifyClicked()));
    ...
    connect(m_logoutButton, SIGNAL(clicked()), this, SLOT(slotLogoutClicked()));
...
{
    // write entries to cvs DBUS service configuration
    KConfigGroup repoGroup = m_serviceConfig->group(QLatin1String("Repository-") + item->repository());

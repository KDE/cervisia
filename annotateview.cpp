...
public:
    ...
    AnnotateViewItem(AnnotateView *parent, const LogInfo &logInfo, const QString &content, bool odd, int linenumber);
...
AnnotateViewItem::AnnotateViewItem(AnnotateView *parent, const LogInfo &logInfo, const QString &content, bool odd, int linenumber)
...
void AnnotateViewDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (item->isSelected() || index.column() == AnnotateViewItem::LineNumberColumn) {
        ...
        backgroundColor = item->m_odd ? KColorScheme(QPalette::Active, KColorScheme::View).background().color()
...
void AnnotateView::slotQueryToolTip(const QPoint &viewportPos, QRect &viewportRect, QString &text)
{
...
void AnnotateView::findText(const QString &textToFind, bool up)
{
    ...
    for (; item && !item->text(AnnotateViewItem::ContentColumn).contains(textToFind, Qt::CaseInsensitive); item = up ? itemAbove(item) : itemBelow(item))
        ;

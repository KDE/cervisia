    ...
    , m_jobType(Unknown)
{
    ...
    if (cvsService) {
        ...
        update->setFocus();
        connect(update, SIGNAL(customContextMenuRequested(QPoint)), this, SLOT(popupRequested(QPoint)));

        connect(update, SIGNAL(fileOpened(QString)), this, SLOT(openFile(QString)));
...
void CervisiaPart::setupActions()
{
    ...
    hint = i18n(
        "Determines whether files with status up-to-date or "
        "unknown are hidden");
    action->setToolTip(hint);
    action->setWhatsThis(hint);
    ...
    if (auto popup = static_cast<QMenu *>(hostContainer(xmlName))) {
        if (isFileItem(item)) {
            ...
            if (!selectedFile.isEmpty()) {
                ...
                if (m_currentEditMenu->menu())
                    m_editWithAction = popup->insertMenu(popup->actions().at(1), m_currentEditMenu->menu());
            }
        }
        ...
        if (xmlName == "noncvs_context_popup") {
            ...
            if (m_currentIgnoreMenu->menu())
                m_addIgnoreAction = popup->insertMenu(actionCollection()->action("file_add"), m_currentIgnoreMenu->menu());
        }
...
void CervisiaPart::updateActions()
{
    ...
    stateChanged("has_single_selection", single ? StateNoReverse : StateReverse);

    bool singleFolder = (update->multipleSelection().count() == 1);
    stateChanged("has_single_folder", singleFolder ? StateNoReverse : StateReverse);
    ...
    stateChanged("has_running_job", hasRunningJob ? StateNoReverse : StateReverse);
}
...
void CervisiaPart::slotStatus()
{
    ...
    QDBusReply<QDBusObjectPath> cvsJobPath = cvsService->simulateUpdate(list, opt_updateRecursive, opt_createDirs, opt_pruneDirs);
...
void CervisiaPart::slotCommit()
{
    ...
    if (dlg.exec()) {
        ...
        QDBusReply<QDBusObjectPath> cvsJobPath = cvsService->commit(list, dlg.logMessage(), opt_commitRecursive);
        QString cmdline;
        QDBusObjectPath cvsJob = cvsJobPath;
        qCDebug(log_cervisia) << " commit: cvsJob.path():" << cvsJob.path();
        qCDebug(log_cervisia) << " list:" << list << "dlg.logMessage():" << dlg.logMessage() << "opt_commitRecursive" << opt_commitRecursive;
        if (cvsJob.path().isEmpty())
            return;
...
void CervisiaPart::updateSandbox(const QString &extraopt)
{
    ...
    QDBusReply<QDBusObjectPath> cvsJobPath = cvsService->update(list, opt_updateRecursive, opt_createDirs, opt_pruneDirs, extraopt);
...
void CervisiaPart::slotImport()
{
    ...
    QDBusReply<QDBusObjectPath> cvsJobPath = cvsService->import(dlg.workingDirectory(),
                                                                dlg.repository(),
                                                                dlg.module(),
                                                                dlg.ignoreFiles(),
                                                                dlg.comment(),
...
void CervisiaPart::slotCheckout()
{
    ...
    QDBusReply<QDBusObjectPath> cvsJobPath =
        cvsService
            ->checkout(dlg.workingDirectory(), dlg.repository(), dlg.module(), dlg.branch(), opt_pruneDirs, dlg.alias(), dlg.exportOnly(), dlg.recursive());
    QDBusObjectPath cvsJob = cvsJobPath;
    QString cmdline;
    OrgKdeCervisia5CvsserviceCvsjobInterface cvsjobinterface(m_cvsServiceInterfaceName, cvsJob.path(), QDBusConnection::sessionBus(), this);
    QDBusReply<QString> reply = cvsjobinterface.cvsCommand();
...
void CervisiaPart::createOrDeleteTag(TagDialog::ActionType action)
{
    ...
    if (dlg.exec()) {
        ...
        if (action == TagDialog::Create)
            cvsJob = cvsService->createTag(list, dlg.tag(), dlg.branchTag(), dlg.forceTag());
        else
            cvsJob = cvsService->deleteTag(list, dlg.tag(), dlg.branchTag(), dlg.forceTag());
        QDBusObjectPath cvsJobPath = cvsJob;
        QString cmdline;
        OrgKdeCervisia5CvsserviceCvsjobInterface cvsjobinterface(m_cvsServiceInterfaceName, cvsJobPath.path(), QDBusConnection::sessionBus(), this);
        QDBusReply<QString> reply = cvsjobinterface.cvsCommand();
...
void CervisiaPart::slotConfigure()
{
    ...
    splitter->setOrientation(splitHorz ? Qt::Vertical : Qt::Horizontal);
    delete l;
}
...
void CervisiaPart::slotJobFinished()
{
    ...
    if (m_jobType == Commit) {
        KNotification::event("cvs_commit_done", i18n("A CVS commit to repository %1 is done", repository), QPixmap(), widget()->parentWidget());
        m_jobType = Unknown;
    }
}
...
bool CervisiaPart::openSandbox(const QUrl &url)
{
    ...
    KConfig *conf = config();
    bool dostatus = conf->group("General").readEntry(repository.contains(":") ? "StatusForRemoteRepos" : "StatusForLocalRepos", false);
    if (dostatus) {
...
void CervisiaPart::readSettings()
{
    ...
    (static_cast<KToggleAction *>(actionCollection()->action("settings_create_dirs")))->setChecked(opt_createDirs);

    opt_pruneDirs = config.readEntry("Prune Dirs", true);
    (static_cast<KToggleAction *>(actionCollection()->action("settings_prune_dirs")))->setChecked(opt_pruneDirs);

    opt_updateRecursive = config.readEntry("Update Recursive", false);
    (static_cast<KToggleAction *>(actionCollection()->action("settings_update_recursively")))->setChecked(opt_updateRecursive);

    opt_commitRecursive = config.readEntry("Commit Recursive", false);
    (static_cast<KToggleAction *>(actionCollection()->action("settings_commit_recursively")))->setChecked(opt_commitRecursive);

    opt_doCVSEdit = config.readEntry("Do cvs edit", false);
    (static_cast<KToggleAction *>(actionCollection()->action("settings_do_cvs_edit")))->setChecked(opt_doCVSEdit);

    opt_hideFiles = config.readEntry("Hide Files", false);
    (static_cast<KToggleAction *>(actionCollection()->action("settings_hide_files")))->setChecked(opt_hideFiles);

    opt_hideUpToDate = config.readEntry("Hide UpToDate Files", false);
    (static_cast<KToggleAction *>(actionCollection()->action("settings_hide_uptodate")))->setChecked(opt_hideUpToDate);

    opt_hideRemoved = config.readEntry("Hide Removed Files", false);
    (static_cast<KToggleAction *>(actionCollection()->action("settings_hide_removed")))->setChecked(opt_hideRemoved);

    opt_hideNotInCVS = config.readEntry("Hide Non CVS Files", false);
    (static_cast<KToggleAction *>(actionCollection()->action("settings_hide_notincvs")))->setChecked(opt_hideNotInCVS);

    opt_hideEmptyDirectories = config.readEntry("Hide Empty Directories", false);
    (static_cast<KToggleAction *>(actionCollection()->action("settings_hide_empty_directories")))->setChecked(opt_hideEmptyDirectories);
